...
export const RecipeCreator: React.FC<RecipeCreatorProps> = ({ onSave, onCancel, lang, pantryItems }) => {
    ...
    // We need to store name for display, but RecipeIngredient uses ID.
    // So we use a local interface for the creator state
    ...
    const handleAddIngredient = () => {
        ...
        // Use selected ID if available and name matches, otherwise use name as ID (custom)
        const finalId = (selectedIngId && ingName === (t.ingredients as any)[selectedIngId])
            ? selectedIngId
            : ingName.trim().toLowerCase().replace(/\s+/g, '_');

        setIngredients([...ingredients, {
            id: finalId,
            name: ingName.trim(),
            amount: ingAmount.trim() || 'to taste'
        }]);
        setIngName('');
        setIngAmount('');
    ...
    const handleSave = () => {
        ...
        const newRecipe: Recipe = {
            ...
            title,
            description,
            ingredients: ingredients.map(i => ({ id: i.id as any, amount: i.amount })),
